...
export function CreateLibraryNotebookModal({
  ...
  async function handleCreate() {
    ...
    setError(null);
    try {
      const nb = await createLibraryNotebook({ title: title.trim(), tags });
    ...
    >
      <div className="bg-vellum rounded-[2px] border border-ink shadow-[4px_4px_0_rgb(42_36_24_/_0.18)] w-full max-w-md flex flex-col overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-4">
          <div>
            <label className="block font-mono text-[10px] tracking-[0.18em] uppercase text-ink-mute mb-2">Title</label>
            <Input
              autoFocus
