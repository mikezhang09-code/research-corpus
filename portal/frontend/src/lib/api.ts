...
/** POST a (possibly empty) id list to a zip endpoint and save the response as a
 *  file. Omitting `ids` (or passing an empty array) downloads everything. The
 *  filename comes from the server's Content-Disposition; `X-Skipped-Count`
 *  reports items left out (e.g. unsaved Corpus artifacts). */
async function downloadZip(path: string, ids?: string[]): Promise<DownloadResult> {
  const res = await fetch(`${BASE}${path}`, {
    method: "POST",
