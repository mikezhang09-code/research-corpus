...
import {
  X, Loader2, Save, AlertCircle, Sparkles, Wand2, Copy, Download,
...
import {
  ...
  getLibraryFileContent, assistDiagram, type LibraryFile,
} from "@/lib/api";
...
// (file_category "diagram", .mmd). `notebookId: null` targets free-forms and
// the AI starts from scratch; a folio id grounds the AI in that folio's files.
export function DiagramEditorModal<T extends { id: string; title: string } = LibraryFile>({
  ...
  const runAssistant = useCallback(async () => {
    ...
    setAiNote("");
    try {
      const res = await assistDiagram(instruction, code, notebookId);
      setCode(res.mermaid);
    ...
    >
      <div className={`bg-vellum rounded-[2px] border border-ink shadow-[4px_4px_0_rgb(42_36_24_/_0.18)] w-full ${expanded ? EXPANDED_MODAL : "max-w-5xl max-h-[90vh]"} flex flex-col overflow-hidden`}>
        ...
        <div className="px-5 py-4 flex flex-col gap-3 flex-1 min-h-0">
          ...
          {/* AI assist bar */}
          <div className="rounded-[2px] border border-rule bg-paper-light p-2.5 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                placeholder={notebookId
                  ? "Ask AI — grounded in this folio's files…"
                  : "Describe the diagram you want…"}
                className="h-9 flex-1 rounded-[1px] border border-rule bg-paper px-3 font-mono text-[12px] text-ink placeholder:text-ink-mute placeholder:italic focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ink"
              />
              <Button onClick={runAssistant} disabled={aiBusy || loading || !prompt.trim()} size="sm" className="gap-1.5 rounded-[1px]">
                {aiBusy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Wand2 className="h-3.5 w-3.5" />}
                {aiBusy ? "Thinking…" : "Generate"}
              </Button>
            </div>
            {aiNote && <p className="mt-1.5 font-mono text-[10px] tracking-[0.06em] text-ink-fade">✺ {aiNote}</p>}
          </div>
