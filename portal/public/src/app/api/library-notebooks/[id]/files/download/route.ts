...
// POST /api/library-notebooks/{id}/files/download  → zip of folio files.
// Body { ids?: string[] }; omit/empty ids = download the whole folio.
export async function POST(req: Request, { params }: RouteParams) {
  ...
  const title = (nb as { title?: string } | null)?.title || "folio";
  return zipResponse(await buildZip(entries), `${title}.zip`);
}
